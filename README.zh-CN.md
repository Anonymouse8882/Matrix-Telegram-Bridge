...
| `!tg check [目标\|all]` | 检查对话在 Telegram 那边还在不在：对方注销账户 → 房名加 `【已注销】`，对话没了 → 加 `（已删除）`；恢复后自动还原 |
| `!tg help` | 显示帮助 |
...
发送失败时不会只说“见日志”：如果 Telegram 明确回复对方账户已注销，会直接告诉你
「对方账户已注销」，并**自动**把该对话的专属房间改名为 `🗑 名字【已注销】`——不必再
手动跑 `check`。房间和聊天记录都保留；对方要是又活了，收到新消息会自动还原房名。
